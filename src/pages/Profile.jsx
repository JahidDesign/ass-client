  ...
  const renderCards = (items, type) => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {items.map((item) => (
        <div
          key={item._id}
          className="bg-white shadow border rounded-lg overflow-hidden flex flex-col"
        >
          <img
          ...
          <div className="p-4 flex-grow flex flex-col">
            <h4 className="text-xl font-bold text-gray-800 mb-1 capitalize">
              {item.selectedPackage || item.hotelName || item.flightNumber || "Booking"}
            </h4>
            {item.pickupLocation && (
              <p className="text-sm text-gray-600 mb-1">📍 {item.pickupLocation}</p>
            )}
            {item.hotelLocation && (
              <p className="text-sm text-gray-600 mb-1">📍 {item.hotelLocation}</p>
            )}
            {item.departure && item.arrival && (
              <p className="text-sm text-gray-600 mb-1">✈️ {item.departure} → {item.arrival}</p>
            )}
            ...
            {item.specialRequests && (
              <p className="text-gray-700 text-sm mb-3 line-clamp-3">{item.specialRequests}</p>
            )}
            {item.mapUrl && (
              <iframe
                src={item.mapUrl}
  ...
  return (
    <div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        ...
        {!loading && (
          <>
            {/* Show only if tour bookings exist */}
            {tourBookings.length > 0 && (
              <div className="mb-12">
                <h3 className="text-2xl font-semibold text-green-700 mb-6">My Tour Bookings</h3>
                {renderCards(tourBookings, "tour")}
              </div>
            )}

            {/* Show only if hotel bookings exist */}
            {hotelBookings.length > 0 && (
              <div className="mb-12">
                <h3 className="text-2xl font-semibold text-blue-700 mb-6">My Hotel Bookings</h3>
                {renderCards(hotelBookings, "hotel")}
              </div>
            )}

            {/* Show only if flight bookings exist */}
            {flightBookings.length > 0 && (
              <div>
                <h3 className="text-2xl font-semibold text-purple-700 mb-6">My Flight Bookings</h3>
            ...
            {tourBookings.length === 0 &&
              hotelBookings.length === 0 &&
              flightBookings.length === 0 && (
                <p className="text-center text-gray-500 mt-12">You have no bookings yet.</p>
              )}
          </>
        )}

        {loading && (
          <p className="text-center text-gray-600 mt-12">Loading your bookings...</p>
        )}
      </div>
    </div>
  );
