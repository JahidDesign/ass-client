    ...
    >
      {bookings.map((b, idx) => (
        <motion.div
          ...
          } ${theme === "dark" ? "bg-gray-800 text-gray-100" : "bg-white text-gray-900"} shadow-lg rounded-2xl p-6`}
          variants={card}
          whileHover="hover"
        >
          {/* Tour Image */}
          <div className="md:w-1/2 w-full rounded-2xl overflow-hidden relative">
            ...
            />
            <div className="absolute top-3 left-3 bg-yellow-500 text-white px-2 py-1 rounded-md text-xs font-semibold shadow">
              Booked
            </div>
          </div>
          ...
          <div className="md:w-1/2 w-full flex flex-col gap-4">
            ...
            {/* Pickup Location */}
            <p className="flex items-center gap-2"><FaMapMarkerAlt className="text-red-500" /> {b.pickupLocation}</p>
            ...
            <div className="flex items-center gap-3 mt-2 border-t pt-3">
              <img
                src={b.userPhoto || "https://via.placeholder.com/40"}
                alt={b.userName}
                className="w-12 h-12 rounded-full border object-cover"
              />
              <div className="flex flex-col">
                <p className="flex items-center gap-1 font-medium"><FaUserCircle className="text-blue-500" /> {b.userName}</p>
                <p className="flex items-center gap-1 text-sm text-gray-400"><FaEnvelope /> {b.userEmail}</p>
              </div>
            </div>
            ...
            {/* Map */}
            {b.mapUrl && (
              <div className="mt-4 w-full h-48 rounded-lg overflow-hidden border">
                <iframe src={b.mapUrl} title="Tour Map" className="w-full h-full" loading="lazy" />
              </div>
            )}
            ...
            <div className="flex gap-3 mt-3">
              ...
              <button
                ...
                className="flex-1 py-2 px-4 bg-red-500 text-white rounded-lg hover:bg-red-600 transition flex items-center justify-center gap-2"
              >
