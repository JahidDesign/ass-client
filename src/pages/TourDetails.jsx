...
const ToursBanner = ({ tour }) => {
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: `url(${tour.photoUrl || tour.photo || "https://placehold.co/1200x800"})`,
      }}
    >
...
export default function TourDetails() {
  ...
  useEffect(() => {
    ...
    fetch(`https://ass-server-1.onrender.com/bookings?email=${user.email}`)
      .then((res) => res.json())
      .then((data) => {
        const booked = data.some((b) => b.tourId === id && b.userEmail === user.email);
  ...
  // Loading state
  if (loading) return <div className="max-w-4xl mx-auto p-6 animate-pulse"><div className="h-72 bg-gray-200 rounded-lg" /></div>;
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 mt-2">
      ...
      <motion.div
        ...
        className="bg-white shadow-xl rounded-2xl overflow-hidden mt-4"
      >
        <div className="md:flex">
          ...
          <div className="md:w-1/2 relative h-80 md:h-full">
            ...
            <div className="absolute left-4 bottom-4 bg-white/80 backdrop-blur px-3 py-1 rounded-lg shadow">
              <p className="text-sm font-semibold">{tour.duration || "Flexible"}</p>
              <p className="text-xs text-gray-600">{tour.category || tour.selectedPackage || "General"}</p>
            </div>
          </div>

          {/* Right Details */}
          <div className="md:w-1/2 p-6 space-y-4">
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900">{tour.title || tour.selectedPackage}</h1>
            <p className="mt-1 text-sm text-gray-600">{tour.location || tour.pickupLocation}</p>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {tour.features?.hotel && <span className="bg-blue-100 text-blue-800 px-2 py-1 rounded text-xs">Hotel</span>}
              {tour.features?.car && <span className="bg-green-100 text-green-800 px-2 py-1 rounded text-xs">Car</span>}
              {tour.features?.boat && <span className="bg-yellow-100 text-yellow-800 px-2 py-1 rounded text-xs">Boat</span>}
              {tour.features?.restaurant && <span className="bg-pink-100 text-pink-800 px-2 py-1 rounded text-xs">Restaurant</span>}
            </div>
            ...
            <div className="flex gap-3 mt-4">
              ...
              <button
                onClick={() => navigate(-1)}
                className="py-3 px-5 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700"
              >
                Back
              </button>
            </div>
            ...
            {tour.specialRequests && (
              <div className="mt-3 p-3 bg-gray-50 rounded text-gray-700 text-sm whitespace-pre-line">
