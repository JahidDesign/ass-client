...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      if (location.pathname.includes("customer")) {
        results = customers.filter(c => c.fullName?.toLowerCase().includes(q) || c.email?.toLowerCase().includes(q)).map(c => ({ type: "Customer", data: c }));
      } else if (location.pathname.includes("hotel")) {
        results = hotels.filter(h => h.hotelName?.toLowerCase().includes(q) || h.hotelLocation?.toLowerCase().includes(q)).map(h => ({ type: "Hotel", data: h }));
      } else if (location.pathname.includes("tour")) {
        results = tours.filter(t => t.selectedPackage?.toLowerCase().includes(q) || t.pickupLocation?.toLowerCase().includes(q)).map(t => ({ type: "Tour", data: t }));
      } else if (location.pathname.includes("flight")) {
        results = flights.filter(f => f.passengerName?.toLowerCase().includes(q) || f.email?.toLowerCase().includes(q)).map(f => ({ type: "Flight", data: f }));
      } else {
        results = [
  ...
  // Navbar styles
  const navbarBg = isScrolled ? (theme === "dark" ? "bg-gray-900 shadow-lg" : "bg-white shadow-lg") : (theme === "dark" ? "bg-gray-800/70 backdrop-blur-md" : "bg-white/70 backdrop-blur-md");
    ...
    <header className={`fixed top-0 left-0 w-full z-50 transition-colors ${navbarBg} border-b`}>
      <div className="flex justify-between items-center px-6 py-3 max-w-7xl mx-auto">
        ...
        <nav className={`hidden md:flex items-center gap-8 ${navTextColor}`}>
          ...
          <Link onClick={() => handleProtectedClick("/all-packages")} className="hover:text-blue-500">AllPackages</Link>

          {/* Package Dropdown */}
          <div className="relative">
            <button onClick={() => { if(!user) return handleProtectedClick("/login"); setPackageDropdownOpen(!packageDropdownOpen); }} className="flex items-center gap-1 hover:text-blue-500">
              AddPackage <FaChevronDown className={`${packageDropdownOpen ? "rotate-180" : ""} transition-transform`} />
            </button>
            <AnimatePresence>
              {packageDropdownOpen && user && (
                <motion.div ref={packageRef} initial={{opacity:0, y:-10}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-10}} transition={{duration:0.2}} className={`absolute top-full mt-2 w-44 shadow rounded z-50 ${dropdownBg}`}>
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          <div className="relative" ref={searchRef}>
            <input type="text" placeholder="Search..." value={searchQuery} onChange={(e)=>setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2 rounded-full border focus:outline-none focus:ring-2 focus:ring-blue-500 w-72"/>
            <FaSearch className="absolute left-3 top-3 text-gray-400"/>
            {searchResults.length>0 && (
              <div className="absolute mt-1 w-full max-h-64 overflow-y-auto shadow-lg rounded bg-white dark:bg-gray-800 z-50">
                {searchResults.map((r,idx)=>(
                  <div key={idx} onClick={()=>handleClickResult(r)} className="flex items-center px-4 py-2 gap-3 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
                    ...
                    <span className={`ml-auto text-xs font-semibold ${r.type==="Customer"?"text-blue-500":r.type==="Hotel"?"text-yellow-500":r.type==="Tour"?"text-green-500":"text-purple-500"}`}>
                      {r.type}
                    </span>
                  </div>
                ))}
          ...
          {user ? (
            <div className="relative">
              <img ref={avatarRef} onClick={()=>setDropdownOpen(!dropdownOpen)} src={user.photoURL || "/default-avatar.png"} alt="avatar" className="w-10 h-10 rounded-full cursor-pointer border-2 border-blue-600"/>
              <AnimatePresence>
                {dropdownOpen && (
                  <motion.div initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}} transition={{duration:0.2}} className={`absolute right-0 mt-2 w-48 shadow rounded z-50 text-sm ${dropdownBg}`}>
      ...
      <AnimatePresence>
        {drawerOpen && (
          <motion.div initial={{x:"100%"}} animate={{x:0}} exit={{x:"100%"}} transition={{duration:0.3}} className={`fixed top-0 right-0 h-full w-64 shadow-lg z-50 ${theme==="dark"?"bg-gray-900 text-white":"bg-white text-gray-900"}`}>
            <div className="flex flex-col p-6 gap-6">
              ...
              {/* Mobile Search */}
              <div className="relative">
                <input type="text" placeholder="Search..." value={searchQuery} onChange={(e)=>setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2 rounded-full border focus:outline-none focus:ring-2 focus:ring-blue-500 w-full"/>
                <FaSearch className="absolute left-3 top-3 text-gray-400"/>
              </div>

              <button onClick={toggleTheme} className="flex items-center gap-2">{theme==="dark"?<FaSun/>:<FaMoon/>} Theme</button>
